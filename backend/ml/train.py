# backend/ml/train.py
"""
Train classification model + isolation forest, compute drift, save artifacts,
and write a ModelMetrics row into DB.
...
def _load_training_data(db):
    rows = db.query(Incident).filter(Incident.priority.isnot(None)).filter(Incident.summary.isnot(None)).all()
...
def compute_drift_score(iforest, vec, recent_texts):
    ...
    drift_score = float(np.clip((1 - (mean + 1) / 2), 0.0, 1.0))
    return drift_score
...
def run_train():
    ...
    try:
        ...
        recent_rows = db.query(Incident).filter(Incident.summary.isnot(None)).order_by(Incident.timestamp.desc().nullslast(), Incident.id.desc()).limit(200).all()
        ...
        metrics = ModelMetrics(timestamp=datetime.utcnow(), model_version=version, accuracy=accuracy, drift_score=drift_score, drift_detected=drift_detected)
        db.add(metrics)
        db.commit()

        persist_drift_state({"model_version": version, "accuracy": accuracy, "drift_score": drift_score, "drift_detected": drift_detected, "timestamp": datetime.utcnow().isoformat()})
        print(f"Trained: accuracy={accuracy:.3f}, drift_score={drift_score:.3f}, drift_detected={drift_detected}")
        return True
